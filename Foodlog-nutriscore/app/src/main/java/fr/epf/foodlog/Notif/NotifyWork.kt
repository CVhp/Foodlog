...
import fr.epf.foodlog.vectorToBitmap
...
class NotifyWork(context: Context, params: WorkerParameters) : Worker(context, params) {
    ...
    override fun doWork(): Result {
        ...
        for(i in Product.all){
            if(i.date.isBefore(limitDa) && i.date.isAfter(d)  || i.date.isEqual(d) || i.date.isEqual(limitDa)){
                sendNotification(id)
            }
        }
    ...
    private fun sendNotification(id: Int) {
        val intent = Intent(applicationContext, ListProductActivity::class.java)
        ...
        val notification = NotificationCompat.Builder(applicationContext, NOTIFICATION_CHANNEL)
            .setLargeIcon(bitmap).setSmallIcon(R.drawable.ic_notifications_white_24dp)
